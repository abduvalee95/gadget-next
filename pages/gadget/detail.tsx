...
import Review from '../../libs/components/gadget/Review';
...
const GadgetDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationGadgets, setDestinationGadgets] = useState<Gadget[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [gadgetComments, setGadgetComments] = useState<Comment[]>([]);
	const [commentTotal, setCommentTotal] = useState<number>(0);
	const [insertCommentData, setInsertCommentData] = useState<CommentInput>({
		commentGroup: CommentGroup.GADGET,
		commentContent: '',
		commentRefId: '',
	});
	...
	const {
		...
		onCompleted: (data: T) => {
			if (data?.getGadget) /* tekshiramiz Getproperoty bormi */ setGadget(data.getGadget);
			if (data?.getGadget) setSlideImage(data.getGadget?.gadgetImages[0]); // slide Imageni qiymati tashkilanadi
		},
	});

	/** BIRXIL LOCATIONDAGI GADGETLARIMIZNI PASDA KORSATISH UCHUN  **/
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: gadget?.gadgetLocation ? [gadget?.gadgetLocation] : [], // locationi teng bolganini olib ber
				},
			},
		},
		//Malumotlar qachon olinish kerak
		skip: !gadgetId && !gadget, // mavjud olmasa skip qiladi agar bolsa qabul qiladi
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getGadgets?.list) setDestinationGadgets(data?.getGadgets?.list); //bunda Gadget larimizni qiymatni ozgartiramiz
		},
	});

	/**Comment GADGETLARIMIZNI PASDA KORSATISH UCHUN  **/
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getComments?.list) setGadgetComments(data?.getComments?.list); //bunda Gadget larimizni qiymatni ozgartiramiz
	...
	useEffect(() => {
		if (router.query.id) {
			setGadgetId(router.query.id as string); //query ichida id boladi
			setCommentInquiry({
				...commentInquiry,
				search: {
	...
	const likeGadgetHandler = async (user: T, id: string) => {
		try {
			...
			await getGadgetRefetch({ input: gadgetId });
			await getGadgetsRefetch({
				input: {
					page: 1,
					limit: 4,
					sort: 'createdAt',
					direction: Direction.DESC,
					search: {
						locationList: [gadget?.gadgetLocation], // locationi teng bolganini olib ber
					},
				},
	...
	if (device === 'mobile') {
		return <div>GADGET DETAIL PAGE</div>;
	} else {
		return (
			<div id={'property-detail-page'}>
				<div className={'container'}>
					<Stack className={'property-detail-config'}>
						<Stack className={'property-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{gadget?.gadgetTitle}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{gadget?.gadgetLocation}</Typography>
										...
										<Stack className={'buy-rent-box'}>
											{/* {gadget?.gadgetBarter && ( 
												<>
													...
													<Typography className={'buy-rent'}>NEW</Typography>
												</>
											)}*/}
											...
											{/* {gadget?.gadgetRent && ( 
												<>
													...
													<Typography className={'buy-rent'}>USED</Typography>
												</>
											)}*/}
										</Stack>
										<Stack className={'divider'}></Stack>
										...
										<Typography className={'date'}>{moment().diff(gadget?.createdAt, 'days')} days ago</Typography>
									</Stack>
									<Stack className={'bottom-box'}>
										<Stack className="option">
											{/* <img src="/img/icons/bed.svg" alt="" /> <Typography>{gadget?.gadgetBeds} bed</Typography> */}
										</Stack>
										<Stack className="option">
											{/* <img src="/img/icons/room.svg" alt="" /> <Typography>{gadget?.gadgetCapacity} room</Typography> */}
										</Stack>
										<Stack className="option">
										{/* 	<img src="/img/icons/expand.svg" alt="" /> <Typography>{gadget?.gadgetSquare} m2</Typography> */}
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{gadget?.gadgetViews}</Typography>
										</Stack>
										<Stack className="button-box">
											{gadget?.meLiked && gadget?.meLiked[0]?.myFavorite ? (
												<FavoriteIcon
													color="primary"
													fontSize={'medium'}
													onClick={() => likeGadgetHandler(user, gadget?._id)}
												/>
											) : (
												<FavoriteBorderIcon
													fontSize={'medium'}
											...
											<Typography>{gadget?.gadgetLikes}</Typography>
										</Stack>
									</Stack>
							...
							<Stack className={'images'}>
								...
								<Stack className={'sub-images'}>
									{gadget?.gadgetImages.map((subImg: string) => {
							...
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											{/* <Typography className={'option-data'}>{gadget?.gadgetBeds}</Typography> */}
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{gadget?.gadgetCapacity}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{moment(gadget?.createdAt).format('YYYY')}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											{/* <Typography className={'option-data'}>{gadget?.gadgetSquare} </Typography> */}
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{gadget?.gadgetType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										...
										<Typography className={'desc'}>{gadget?.gadgetDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>Gadget Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Price</Typography>
													<Typography className={'data'}>${formatterStr(gadget?.gadgetPrice)}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Size</Typography>
													{/* <Typography className={'data'}>{gadget?.gadgetSquare} mm</Typography> */}
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>GB</Typography>
													<Typography className={'data'}>{gadget?.gadgetCapacity}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Weight</Typography>
													{/* <Typography className={'data'}>{gadget?.gadgetBeds}</Typography> */}
												</Box>
											</Stack>
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Year</Typography>
													<Typography className={'data'}>{moment(gadget?.createdAt).format('YYYY')}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Model</Typography>
													<Typography className={'data'}>{gadget?.gadgetType}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}> New or Used</Typography>
													<Typography className={'data'}>
														 {/* {gadget?.gadgetBarter && 'Barter'} {gadget?.gadgetRent && 'Rent'} */}
													</Typography>
												</Box>
											</Stack>
										</Stack>
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{gadgetComments?.map((comment: Comment) => {
								...
								<Stack className={'info-box'}>
									...
									<Stack className={'image-info'}>
										<img
											...
											src={
												gadget?.memberData?.memberImage
													? `${REACT_APP_API_URL}/${gadget?.memberData?.memberImage}`
													: '/img/profile/defaultUser.svg'
											}
										/>
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${gadget?.memberData?._id}`}>
												<Typography className={'name'}>{gadget?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{gadget?.memberData?.memberPhone}</Typography>
											</Stack>
						...
						{destinationGadgets.length !== 0 && (
							<Stack className={'similar-properties-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Destination Gadget</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationGadgets.map((gadget: Gadget) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={gadget.gadgetTitle}>
													<GadgetBigCard gadget={gadget} likeGadgetHandler={likeGadgetHandler} key={gadget?._id} />{' '}
...
export default withLayoutFull(GadgetDetail);
