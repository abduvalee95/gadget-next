...
import { CREATE_GADGET, UPDATE_GADGET } from '../../../apollo/user/mutation';
import { getJwtToken } from '../../auth';
import { REACT_APP_API_URL, } from '../../config';
import { GadgetLocation, GadgetType } from '../../enums/gadget.enum';
import useDeviceDetect from '../../hooks/useDeviceDetect';
import { sweetErrorHandling, sweetMixinErrorAlert, sweetMixinSuccessAlert } from '../../sweetAlert';
import { GadgetInput } from '../../types/gadget/gadget.input';
import { GET_PROPERTY } from '../../../apollo/user/query'

const AddGadget = ({ initialValues, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const inputRef = useRef<any>(null);
	const [insertGadgetData, setInsertGadgetData] = useState<GadgetInput>(initialValues);
	...
	/** APOLLO REQUESTS **/
	// let getGadgetData: any, getGadgetLoading: any;
	const [createGadget] = useMutation(CREATE_GADGET);
	...
	useEffect(() => {
		setInsertGadgetData({
			...insertGadgetData,
			gadgetTitle: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetTitle : '',
			gadgetPrice: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetPrice : 0,
			gadgetType: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetType : '',
			gadgetLocation: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetLocation : '',
			gadgetColor: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetColor : '',
			// gadgetNew: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetNew : false,
	/* 		gadgetUsed: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetUsed : false, */
			gadgetCapacity: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetCapacity : 0,
			gadgetWeight: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetWeight : 0,
			// gadgetDisplaySquare: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetDisplaySquare : 0,
			gadgetDesc: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetDesc : '',
			gadgetImages: getGadgetData?.getGadget ? getGadgetData?.getGadget?.gadgetImages : [],
		});
	}, [getGadgetLoading, getGadgetData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertGadgetData({ ...insertGadgetData, gadgetImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			insertGadgetData.gadgetTitle === '' ||
			insertGadgetData.gadgetPrice === 0 || // @ts-ignore
			insertGadgetData.gadgetType === '' || // @ts-ignore
			insertGadgetData.gadgetLocation === '' || // @ts-ignore
			insertGadgetData.gadgetColor === '' || // @ts-ignore
			// insertGadgetData.gadgetNew === '' || // @ts-ignore
			// insertGadgetData.gadgetUsed === '' ||
			insertGadgetData.gadgetCapacity === 0 ||
			insertGadgetData.gadgetWeight === 0 ||
			// insertGadgetData.gadgetDisplaySquare === 0 || 
			insertGadgetData.gadgetDesc === '' ||
			insertGadgetData.gadgetImages.length === 0
		) {
			return true;
		}
	};

	const insertGadgetHandler = useCallback(async () => {
		try {
			const result = await createGadget({
				variables: {
					input: insertGadgetData,
				},
			});
			await sweetMixinSuccessAlert('This Gadget Succsessfully Created ');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myGadgets',
	...
	const updateGadgetHandler = useCallback(async () => {
		try {
			//@ts-ignore
			insertGadgetData._id = getGadgetData?.getGadget?._id;
			const result = await updateGadget({
				variables: {
					input: insertGadgetData,
				},
			});
			await sweetMixinSuccessAlert('This Gadget Updated Succsessfully ');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myGadgets',
	...
	console.log('+insertGadgetData', insertGadgetData);

	if (device === 'mobile') {
		return <div>ADD NEW GADGET MOBILE PAGE</div>;
	} else {
		return (
			<div id="add-property-page">
				<Stack className="main-title-box">
					<Typography className="main-title">Add New Gadget</Typography>
					...
					<Stack className="config">
						<Stack className="description-box">
							<Stack className="config-column">
								...
								<input
									...
									placeholder={'Title'}
									value={insertGadgetData.gadgetTitle}
									onChange={({ target: { value } }) =>
										setInsertGadgetData({ ...insertGadgetData, gadgetTitle: value })
									}
								/>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Price'}
										value={insertGadgetData.gadgetPrice}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetPrice: parseInt(value) })
										}
									/>
								</Stack>
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertGadgetData.gadgetType || 'select'}
										value={insertGadgetData.gadgetType || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertGadgetData({ ...insertGadgetData, gadgetType: value })
										}
									>
										<>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										defaultValue={insertGadgetData.gadgetLocation || 'select'}
										value={insertGadgetData.gadgetLocation || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertGadgetData({ ...insertGadgetData, gadgetLocation: value })
										}
									>
										<>
											<option selected={true} disabled={true} value={'select'}>
											...
											{gadgetLocation.map((location: any) => (
								...
								<Stack className="price-year-after-price">
									<Typography className="title">gadgetColor</Typography>
									<input
										type="text"
										className="description-input"
										placeholder={'gadgetColor'}
										value={insertGadgetData.gadgetColor}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetColor: value })
										}
									/>
								</Stack>
							</Stack>
							...
							<Stack className="config-row">
								{/* <Stack className="price-year-after-price">
									<Typography className="title">New</Typography>
									<select
										className={'select-description'}
										value={insertGadgetData.gadgetNew ? 'yes' : 'no'}
										defaultValue={insertGadgetData.gadgetNew ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetNew: value === 'yes' })
										}
									>
								...
								{/* <Stack className="price-year-after-price">
									<Typography className="title">Used</Typography>
									<select
										className={'select-description'}
										value={insertGadgetData.gadgetUsed ? 'yes' : 'no'}
										defaultValue={insertGadgetData.gadgetUsed ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetUsed: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									<Typography className="title">Memory</Typography>
									<select
										className={'select-description'}
										value={insertGadgetData.gadgetCapacity || 'select'}
										defaultValue={insertGadgetData.gadgetCapacity || 'select'}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetCapacity: parseInt(value) })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertGadgetData.gadgetWeight || 'select'}
										defaultValue={insertGadgetData.gadgetWeight || 'select'}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetWeight: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{[500, 400, 600, 700, 450].map((weight: number) => (
								...
								<Stack className="price-year-after-price">
									{/* <Typography className="title">DisplaySquare</Typography>
									<select
										className={'select-description'}
										value={insertGadgetData.gadgetDisplaySquare || 'select'}
										defaultValue={insertGadgetData.gadgetDisplaySquare || 'select'}
										onChange={({ target: { value } }) =>
											setInsertGadgetData({ ...insertGadgetData, gadgetDisplaySquare: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
									{/* 	{gadgetDisplaySquare.map((display: number) => {
											if (display !== 0) {
							...
							<Typography className="property-title">Gadget Description</Typography>
							<Stack className="config-column">
								...
								<textarea
									...
									value={insertGadgetData.gadgetDesc}
									onChange={({ target: { value } }) =>
										setInsertGadgetData({ ...insertGadgetData, gadgetDesc: value })
									}
								></textarea>
							</Stack>
						</Stack>

						<Typography className="upload-title">Upload photos of your gadget</Typography>
						<Stack className="images-box">
							...
							<Stack className="gallery-box">
								{insertGadgetData?.gadgetImages.map((image: string) => {
						...
						<Stack className="buttons-row">
							{router.query.gadgetId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updateGadgetHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertGadgetHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
