...
const MyArticles: NextPage = ({ initialInput, ...props }: T) => {
	...
	const likePropertyHandler = async (e: any, user: any, id: string) => {
		try {
			e.stopPropagation();
			if (!id) return;
			...
			await boardArticleRefetch({ input: initialInput });

			await sweetTopSmallSuccessAlert('Liked', 800);
		} catch (error: any) {
			console.log('Error, LikeproperotyHandler:', error.message);
